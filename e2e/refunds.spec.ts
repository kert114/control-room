...
test.describe("refunds workflow (variant B)", () => {
  test.describe.configure({ mode: "serial" });
  ...
  test("explains blocked decisions before the click and decides inline", async ({ page }) => {
    await signIn(page, ACCOUNTS.approver);
    await page.goto("/refunds/compare");
    const rows = page.locator("table").first().locator("tbody tr");
    await expect(rows.filter({ hasText: "RFD-5005" })).toContainText("Needs escalation");
    await rows.filter({ hasText: "RFD-5005" }).click();
    await expect(page).toHaveURL(/\/refunds\/compare\?.*refund=/);
    ...
    await rows.filter({ hasText: "RFD-5001" }).click();
    await page.getByRole("button", { name: "Reject…" }).click();
    await page.getByRole("button", { name: "Reject refund" }).click();
    await expect(page.getByRole("alert").filter({ hasText: "at least 5 characters" })).toHaveCount(1);
    await page.getByRole("button", { name: "Cancel" }).click();
    await page.getByRole("button", { name: "Approve refund" }).click();
    await expect(page.getByText("Refund RFD-5001 approved")).toBeVisible();
    await expect(page.getByRole("link", { name: /Next open refund/ })).toBeVisible();
    if (test.info().project.name === "narrow") {
      await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true);
    }
  });

  test("keeps the auditor read only in variant B", async ({ page }) => {
    await signIn(page, ACCOUNTS.auditor);
    await page.goto("/refunds/compare");
    await page.locator("table").first().locator("tbody tr").filter({ hasText: "RFD-5005" }).click();
    await expect(page.getByText(/Read only/)).toBeVisible();
